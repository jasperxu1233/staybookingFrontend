...
export const getReservations = () => {
    const authToken = localStorage.getItem("authToken");  //当前用户的信息都存在了loadStorage里面了
...
export const searchStays = (query) => {
    ...
    searchStaysUrl.searchParams.append("lat", 37);  //这边直接hardcode经纬度
    searchStaysUrl.searchParams.append("lon", -122); //想要搜哪的经纬度，就改成哪的经纬度
...
export const uploadStay = (data) => {
    ...
    return fetch(uploadStayUrl, {
        method: "POST",
        headers: {
            Authorization: `Bearer ${authToken}`, //使用token替代了host来做验证，token都存在localStorage里面了。
        },
        body: data,
    }).then((response) => {
        if (response.status !== 200) {
