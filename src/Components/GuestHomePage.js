...
import {message, Tabs, List, Typography, Form, InputNumber, DatePicker, Button, Card, Carousel, Image, Modal} from "antd";
...
class CancelReservationButton extends React.Component {
    ...
    handleCancelReservation = async () => {
        ...
        try {
            ...
            onCancelSuccess();  //cancel掉了reservation之后，会调用父组件的loadData使父组件重新向后端拉取数据而重新刷新reservation列表。
        } catch (error) {
    ...
    render() {
        ...
        return (
            <>
                ...
                >
                    <Form
                        preserve={false}  //在book的时候，要求传入stay时候的信息-->日期
                        labelCol={{ span: 8 }}
                        wrapperCol={{ span: 16 }}
    ...
    render() {
        return (                  //inline表示尽量排在一行的操作
            ...
            <Form onFinish={this.search} layout = "inline">
                ...
                <Form.Item
                    label="Checkin Date"
                    name="checkin_date"    //datapicker会出现一个日历表的日期选择
                    rules={[{ required: true }]}
                >
                    <DatePicker />
                </Form.Item>
               ...
               <Form.Item>
                ...
                <List style={{ marginTop: 20 }}
                      ...
                      renderItem={(item) => (  //搜索到之后把信息一个一个以card心事放出来
                          <List.Item>
                              <Card
                                  key={item.id}
                                  title={
                                  ...
                                  } //StayDetailInfoButton显示stay信息，BookStayButton就是转到预定组件
...
class MyReservations extends React.Component {
    ...
    loadData = async () => {
        ...
        try {
            const resp = await getReservations();  //从后端拿到用户已经预定的stay
            this.setState({
                data: resp,  //拉取数据成功之后放进data
            });
        } catch (error) {
            message.error(error.message);
        } finally {
        ...
        return (
            <List
                ...
                renderItem={(item) => (
                    ...
                    >
                        <List.Item.Meta
                            title={<Text>{item.stay.name}</Text>}  //在guest界面，显示出已经预定的stay的名字
                            description={
                                <>
